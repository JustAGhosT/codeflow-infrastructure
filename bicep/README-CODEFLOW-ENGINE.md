# Azure Infrastructure for CodeFlow Engine

This directory contains the Azure Bicep infrastructure definitions for deploying the CodeFlow Engine application to Azure Container Apps.
...
- **Azure Container Apps**: Serverless container hosting for the CodeFlow Engine
...
All resources follow the pattern: `{env}-codeflow-{region}-{resource}`
...
- `prod-codeflow-san-app` - Production Container App
- `prod-codeflow-san-env` - Container Apps Environment
- `prod-codeflow-san-postgres` - PostgreSQL server
- `prod-codeflow-san-redis` - Redis cache
- `prod-codeflow-san-logs` - Log Analytics workspace

## Prerequisites

1. Azure CLI installed and configured
2. Appropriate Azure subscription permissions
3. Resource group created: `prod-rg-san-codeflow`
...
2. **Generate passwords**:
   ```bash
   POSTGRES_LOGIN="codeflow"  # Or your custom username
...
- `customDomain`: Custom domain for the container app (default: app.codeflow.io)
- `containerImage`: Full container image path
- `postgresLogin`: PostgreSQL administrator username (default: codeflow)
...
- `POSTGRES_DB`: Database name (codeflow)
- `POSTGRES_USER`: Database user (configured via `postgresLogin` parameter, default: codeflow)
...
az postgres flexible-server show \
  --name prod-codeflow-san-postgres-<unique-suffix> \
  --resource-group prod-rg-san-codeflow \
  --query "{fqdn:fullyQualifiedDomainName,adminUser:administratorLogin}" \
  --output json

# Connect using psql
psql -h <fqdn> -U <postgres_login> -d codeflow
```

**Note:** The PostgreSQL username is configured via the `postgresLogin` parameter (defaults to 'codeflow'). For production deployments using GitHub Actions, set the `CODEFLOW_POSTGRES_LOGIN` secret to customize the username.
...
2. Or use Log Analytics workspace: `prod-codeflow-san-logs`
...
1. Check logs: `az containerapp logs show --name prod-codeflow-san-app --resource-group prod-rg-san-codeflow`
...
1. **Configure custom domain DNS records** (REQUIRED before deployment):
   - Add a CNAME record for your custom domain (e.g., app.codeflow.io) pointing to the Container App FQDN
   - You may need to deploy once without the custom domain first, then update DNS, then redeploy
   - Obtain the FQDN from deployment outputs: `az deployment group show --resource-group prod-rg-san-codeflow --name codeflow-engine --query properties.outputs.containerAppUrl.value`
   - Example DNS record: `CNAME app.codeflow.io -> prod-codeflow-san-app.<region>.azurecontainerapps.io`
