...
RESOURCE_GROUP="${RESOURCE_GROUP:-prod-rg-san-codeflow}"
ENV_NAME="${ENV_NAME:-prod-codeflow-san-env}"
CUSTOM_DOMAIN="${CUSTOM_DOMAIN:-app.codeflow.io}"
...
if ! command -v az &> /dev/null; then
    echo "âŒ Error: Azure CLI is not installed"
...
if ! az account show &> /dev/null; then
    echo "âŒ Error: Not logged in to Azure"
...
if ! command -v jq &> /dev/null; then
    echo "âš ï¸ Warning: jq is not installed. Using alternative parsing method."
    USE_JQ=false
else
    USE_JQ=true
fi
...
if ! az containerapp env show -n "$ENV_NAME" -g "$RESOURCE_GROUP" &>/dev/null; then
    echo "â„¹ï¸ Environment '$ENV_NAME' does not exist yet"
    echo "No certificates to clean up. You can proceed with deployment."
    exit 0
fi

echo "âœ… Environment exists, checking for managed certificates..."
echo ""
...
if [ "$CERTS_OUTPUT" == "[]" ]; then
    echo "â„¹ï¸ No certificates found in environment"
...
if [ "$USE_JQ" = true ]; then
    ...
    if [ -n "$DUPLICATE_CERTS" ]; then
        echo "âš ï¸  WARNING: jq is not available - cannot automatically filter certificates by domain!"
...
if [ -z "$DUPLICATE_CERTS" ] || [ "$DUPLICATE_CERTS" == "null" ]; then
    echo "âœ… No duplicate managed certificates found for domain: $CUSTOM_DOMAIN"
    echo "You can proceed with deployment."
    exit 0
fi

# Display found certificates
echo "âš ï¸ Found duplicate managed certificate(s) for domain: $CUSTOM_DOMAIN"
echo ""
...
if [ -t 0 ] && [ "$CI" != "true" ]; then
    ...
    if [ "$confirm" != "yes" ]; then
        echo "âŒ Cleanup cancelled by user"
        exit 1
    fi
fi

# Delete certificates
echo "ðŸ—‘ï¸ Deleting duplicate certificates..."
echo ""
...
if [ $FAILED_COUNT -eq 0 ]; then
    echo "âœ… All duplicate certificates removed successfully"
    echo "You can now proceed with your deployment."
else
    echo "âš ï¸ Some certificates could not be deleted"
